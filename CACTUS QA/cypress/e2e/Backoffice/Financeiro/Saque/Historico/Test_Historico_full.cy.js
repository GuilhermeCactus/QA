describe('Teste Saque > Historico Full',() => {
    ...
    opc_status.forEach((opc) => {
        it(`Testando filtro para status: ${opc}`, () => {
            ...
            // Selecionando Histórico na opção de Saque
            cy.xpath('//div[@id="withdraws"]/div[@class="menu-item"]/a/span[@class="menu-title" and text() = "Histórico"]')
                .should('be.visible')
                .click();

            // Validando URL
            cy.url().should('contain', 'withdraws/History');
            cy.wait(1000)
                                ...
                                cy.wrap($operador).invoke('text').then((operadorInserido) => {
                                    ...
                                    cy.get('body').then(($body) => {
                                        ...
                                        if ($body.find('.dataTables_empty > .border-0').length > 0) {
                                            ...
                                            for (let i = 1; i <= 6; i++) {
                                                cy.xpath(`//*[@id="kt_app_content_container"]/div//div/div[1]/table/tbody/tr/td[${i}]/div`)
                                                .then(($cell) => {
